...
- `stock_data/io_utils.py` — the only module that reads/writes `skip_symbols.csv`: `read_symbols()` (the working universe, `all_symbols.txt` minus the currently-binding skip rows — prefer this over reading the symbols file directly), the `SkipRow` type (`symbol`, `reason`, `date`), `read_skip_rows()`/`write_skip_rows()`, `append_skip_symbols()` (adds rows without disturbing other reasons; re-dates same-reason rows), `remove_skip_symbols()`, and `is_expired()`. Only `yahoo_unknown` rows expire — after `YAHOO_UNKNOWN_RETRY_DAYS` (30) — so structural classifications are permanent.
...
- `scripts/get_stock_data.py` — the daily entry point (see Storage below). Budgeted: at most `MAX_YAHOO_CALLS_PER_RUN` Yahoo calls per run, missing symbols first, then stalest, skipping anything fresher than `MIN_REFRESH_AGE_DAYS`. A symbol Yahoo returns no fundamentals for is appended to `skip_symbols.csv` as `yahoo_unknown` (it gets no DB row, so it would otherwise stay in `missing` and burn a call every run); that verdict expires after 30 days for one retry, and a successful retry removes the row. A fetch that *raises* is transient and is never skip-listed.
...
- `data/skip_symbols.csv` — generated skip list (`symbol,reason,date`) consumed by `read_symbols()`. Gitignored: the rebuild regenerates it and the daily run appends to it, so it is not source. Reads tolerate the older two-column format.
